@if (session()->get('notify.model') === 'toast')
    <div
        class="notify fixed inset-0 flex items-end justify-end px-4 py-6 pointer-events-none sm:p-6 sm:items-start sm:justify-end">
        <div x-data="{ show: @if (session()->get('notify.model') === 'toast') true @else false @endif }" x-init="setTimeout(() => { show = true }, 500)" x-show="show"
                ...
                <div class="p-4">
                    <div class="flex items-start">
                        ...
                        @if (session()->get('notify.type') === 'warning')
                            <svg class="w-6 h-6 text-yellow-500" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round"
